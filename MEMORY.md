...
- [归因四条件测试验数对候选，不对入围](lessons/2026-09-12-归因四条件测试验数对候选不对入围.md) — 先确认 flag 入围大于 0；结果行数对候选数
- [归因链路关联看候选（同 app 同 IP），不是安装 device JOIN](lessons/2026-09-12-归因链路关联看候选同app同IP-不是安装deviceJOIN.md) — 沿用的关联是落地页同 app 同 IP
...
- [pipeline-runner test 四步与 test_sandbox 铁律：禁动 full_chain；--dt=任务日；已定 runner 禁框外串步](lessons/2026-09-10-pipeline-runner-test四步与test_sandbox铁律.md) — bus#8354
...
- [注册人数业务日=事件时间归属；读 T/T+1 两分区，入仓时间卡次日 0](lessons/2026-09-09-注册人数业务日-事件时间归属-读-t-t-1-两分区-入仓时间卡次日-03-00-截止.md) — 会话轮换蒸馏
- [新增账号活跃少算：先对用户维注册时间做覆盖核查，确认是日批缺口再查命名/](lessons/2026-09-09-新增账号活跃少算-先对用户维注册时间做覆盖核查-确认是日批缺口再查命名-字段.md) — 会话轮换蒸馏
...
- [G5「库内违规=0」≠ 应用层已落地；复验须分开报库内数据与 servi](lessons/2026-08-29-g5-库内违规-0-应用层已落地-复验须分开报库内数据与-service-router-是否做-.md) — 会话轮换蒸馏
...
- [日报只写当天真实完成与真实进度；禁止硬凑](lessons/2026-08-27-日报只写当天真实完成与真实进度禁止硬凑与跟别人回执.md) — 主人 08-27：禁硬凑、不跟别人回执写结果
- [feedback 日报禁硬凑](feedback_daily_report_no_padding.md) — 只写当天完成与真实进度
- [TG 重推日报前先确认推送脚本已改为只发正文无标题头，推完请主人在私聊目](lessons/2026-08-25-tg-重推日报前先确认推送脚本已改为只发正文无标题头-推完请主人在私聊目视验收.md) — 会话轮换蒸馏
...
- [commit 说明须含汉字](lessons/2026-09-10-commit-message-must-be-chinese.md) — 可 feat(scope): 前缀，冒号后中文；禁整段英文；钩子+PINNED §25
...
- [写日报前必须先 prod/test/平台逐项核查再落稿，状态与补数范围禁](lessons/2026-08-07-写日报前必须先-prod-test-平台逐项核查再落稿-状态与补数范围禁止凭印象或局部样本.md) — 会话轮换蒸馏
...
- [群聊仅显式@才回](feedback_group_reply_only_explicit_at.md) — 只有 @初儿/@又初/@youchu_* 才回；裸写又初/只@别人一律静默。
- [群聊仅显式at初儿又初才回](lessons/2026-07-28-群聊仅显式at初儿又初才回.md) — 废止裸喊名；修「又初→狂人」误秒回。
...
- [群聊问进度禁罐头 ACK](lessons/2026-07-24-progress-ask-no-instant-ack.md) — 「进度如何了」→ 直连短报；禁「行，我来」
...
- [日报只写已完成；死锁少写；推送仅 old-mac](feedback_daily_report_no_in_progress.md) — 今日结果只写已完成；未完进明日；死锁尽量空；自动推送仅旧 Mac。
- [日报定稿后推送 TG 私聊](feedback_daily_report_push_tg_dm.md) — old-mac 跑 `memory/scripts/post_daily_report_to_dm.py`。
